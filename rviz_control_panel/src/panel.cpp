  ...
  : rviz_common::Panel(parent)
{
  ...
  {
    auto *title = new QLabel("Inspect coordinate:");
    vbox->addSpacing(8);
    ...
    btn_inspect_exec_ = new QPushButton("Button\n(execute)");
...
void ControlPanel::onReturnBaseClicked()
{
  ...
  opts.result_callback = [this](auto) {
    QMetaObject::invokeMethod(status_, [this]{ status_->setText("RTB goal reached"); });
  };
...
void ControlPanel::onManualControlToggled(bool enabled)
{
  ...
  if (estop_pub_) {
    std_msgs::msg::Bool msg; msg.data = true; estop_pub_->publish(msg);
  }
...
{
  if (!nav_client_) { status_->setText("Nav2 client not initialized"); return; }
  ...
  auto opts = rclcpp_action::Client<NavigateToPose>::SendGoalOptions();
  opts.result_callback = [this](auto) {
    QMetaObject::invokeMethod(status_, [this]{ status_->setText("Inspect goal reached"); });
  };
  nav_client_->async_send_goal(goal, opts);
...
void ControlPanel::load(const rviz_common::Config &config)
{
  ...
  float v;
  if (config.mapGetFloat("home_x", &v)) home_x_ = v;
  if (config.mapGetFloat("home_y", &v)) home_y_ = v;
  if (config.mapGetFloat("home_yaw_deg", &v)) home_yaw_deg_ = v;

  QString sx, sy;
  if (config.mapGetString("inspect_x", &sx) && edit_x_) edit_x_->setText(sx);
  if (config.mapGetString("inspect_y", &sy) && edit_y_) edit_y_->setText(sy);
}
} // namespace rviz_control_panel
